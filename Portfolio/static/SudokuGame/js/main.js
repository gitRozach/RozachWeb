...
function createSudokuCell(id, className, text) {
...
function drawGrid() {
    ...
    for (var i = 0; i < BOARD_LENGTH * BOARD_LENGTH; ++i) {
        ...
        if (value == null) {
            gridList.appendChild(createSudokuCell(`cell-${i}`, 'editable-cell', ''));
        } else {
            gridList.appendChild(createSudokuCell(`cell-${i}`, 'base-cell', `${value}`));
        }   
    }
}
...
function startTimer() {
    document.getElementById('time-button').innerHTML = toHHMMSS(secondsPassed);
...
function mouseClicked(event) {
    ...
    } else if (targetIdStr == 'time-button') {
        paused = !paused;
        if (paused) {
            document.getElementById('time-button').innerHTML = 'Continue';
        } else {
            document.getElementById('time-button').innerHTML = toHHMMSS(secondsPassed);
        }
...
function mouseMoved(event) {
    var relY = Math.round((event.y / window.innerHeight) * 25);
    document.getElementsByClassName('sudoku-grid')[0].style.background = `linear-gradient(to bottom, #3399ff ${relY}%, #003366 100%)`;
}
...
function main() {
    sudoku = new SudokuBoard(BOARD_1);
