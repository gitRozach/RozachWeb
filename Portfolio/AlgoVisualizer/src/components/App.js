...
function SudokuCell(id, className, text) {
    return (
...
function SudokuGrid() {
    ...
    for (var i = 0; i < (9 * 9); ++i) {
        ...
        if (value == null) {
            cells.push(SudokuCell(`cell-${i}`, 'editable-cell', ''));
        } else {
            cells.push(SudokuCell(`cell-${i}`, 'base-cell', `${value}`));
        }   
    }
...
function App() {
    ...
    return ( 
        <div id="root">
            ...
            <div id="sudoku-grid"><ul id="grid-list">{SudokuGrid()}</ul></div>
        </div>
    );
}
